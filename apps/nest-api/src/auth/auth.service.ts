...
export class AuthService {
  ...
  async login(
    ...
    TokenResponseDto | { tempToken: string; requiresTwoFactor: true }
  > {
    const { email, password } = loginDto;

    this.logger.logBusiness('LOGIN_ATTEMPT', { email }, 'AuthService');
    ...
    if (user?.lockExpires && user.lockExpires > new Date()) {
      const remainingMinutes = Math.ceil(
        (user.lockExpires.getTime() - Date.now()) / 60000,
      );
      this.logger.warn(
    ...
    if (!user || !(await bcrypt.compare(password, user.password))) {
      ...
      if (user) {
        ...
        if (attempts >= maxAttempts) {
          ...
          throw new UnauthorizedException(
            `Too many failed attempts. Account locked for ${lockoutTime} minutes.`,
          );
        }
  ...
  async signup(createUserDto: CreateUserDto): Promise<TokenResponseDto> {
    ...
    this.logger.logBusiness('SIGNUP_ATTEMPT', { email, name }, 'AuthService');
  ...
  async generateTokens(user: any): Promise<TokenResponseDto> {
    ...
    refreshExpiresAt.setDate(
      refreshExpiresAt.getDate() + refreshExpirationDays,
    );
  ...
  private async sendVerificationEmail(user: UserEntity): Promise<void> {
    ...
    try {
      await this.mailService.sendMail({
        to: user.email,
        subject: '请验证您的邮箱地址 - Please verify your email',
